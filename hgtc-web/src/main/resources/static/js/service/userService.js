...
app.service('userService',function($http){
	...
	//查询实体
	this.selectByPrimaryKey=function(){
		// alert(sessionStorage.getItem("accessToken"));
		return $http.get('member/getInfo'/*, {
	...
	this.add=function(entity){
		...
		return  $http.post('member/register',entity );
	}
    //用户登录
    this.login=function(entity){
        // alert(JSON.stringify(entity));
        return  $http.post('member/login',entity );
    }
	//修改 
	this.updateUserInfo=function(entity){
    	console.log(entity);
		return  $http.post('member/updateInfo',entity );
	}
	//删除
